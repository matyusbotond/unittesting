...
describe('TodoListHeader', () => {
  describe('render', () => {
    it('item count label', () => {
      // Arrange      
      // Act
      const rendered = render(<TodoListHeader itemCount={0} newButtonPressed={jest.fn()} />);
    ...
    it('item count', () => {
      // Arrange      
      // Act
      const rendered = render(<TodoListHeader itemCount={1} newButtonPressed={jest.fn()} />);
    ...
    it('new button', () => {
      ...
      // Assert
      const label = rendered.getByA11yLabel('newButton');
      expect(label.props.children[0].props.children.props.children).toBe('Új');
    });
  ...
  describe('must call', () => {
    it('newButtonPressed function if newButton pressed', () => {
      ...
      const rendered = render(<TodoListHeader itemCount={0} newButtonPressed={newButtonPressedMock}/>);
